...
function doAll() {
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            const d = new Date(); // new Date() function give a current date when loggin
    ...
    .get()   //the collection called "hikes"
        .then(allHikes => {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                var docID = doc.id;                 // get the unique id for the document eachHike
                ...
                newcard.querySelector('i').onclick = () => saveBookmark(docID); //event listeners

                //checked if the hike is already bookmarked
                // go to the user's document and check if the bookmarks array inluced the hike's docID
                // if so display the filled bookmark
                currentUser.get().then(userDoc => {
                    //get the user name
